...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(self,
                 ...
                 attr_algo: str = None, train_shuffle: bool = False, train_rationale_selection: str = None,annotation=None,data_type=None,
                 ):
        super().__init__()
    ...
    def load_dataset(self, split):
        ...
        for key in tqdm(data_keys, desc=f'Loading {split} set'):
            if key == 'rationale_indices' and split == 'train' and self.pct_train_rationales is not None:
                ...
                if self.annotation=='lexicon_based':
                    filename=f'{key}_lexicon_based.pkl'
                elif self.annotation=='lexicon_based_all':
                    filename=f'{key}_lexicon_based_all.pkl'
                elif self.annotation=='instance_based':
                    filename = f'{key}_instance_based.pkl'
                else:
                    if self.dataset=='imdb':
                        filename=f'{key}_{self.data_type}.pkl'
                    else:
                        filename = f'{key}.pkl'
...
class TextClassificationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        has_rationale = torch.LongTensor([self.data['has_rationale'][idx]]) if self.data.get('rationale') else None
    ...
    def collater(self, items):
        ...
        batch = {
            ...
            'has_rationale': torch.cat([x[4] for x in items]) if self.data.get('rationale') else None,
