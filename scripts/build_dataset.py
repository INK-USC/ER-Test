...
def select_rationale_indices(data_path, dataset_dict, num_examples, train_rationale_selection, pct_train_rationales, seed=None):
...
def main(args):
    ...
    if args.anno_method=='lexicon_based':
        ...
        missing_data_keys = [x for x in data_keys if not os.path.exists(os.path.join(data_path, f'{x}_lexicon_based.pkl'))]
    elif args.anno_method=='lexicon_based_all':
        missing_data_keys = [x for x in data_keys if not os.path.exists(os.path.join(data_path, f'{x}_lexicon_based_all.pkl'))]
    elif args.anno_method=='instance_based':
        missing_data_keys = [x for x in data_keys if not os.path.exists(os.path.join(data_path, f'{x}_instance_based.pkl'))]
    else:
    ...
    if args.num_samples is None and missing_data_keys:
        ...
        if args.dataset in eraser_datasets:
            ...
            sst5_labels = [max(0, math.ceil(dataset[i]['label'] * sst5_num_classes) - 1) for i in range(sst5_num_examples)]
            ...
            for idx, sst5_idx in tqdm(enumerate(sst5_indices), desc=f'Building {args.split} dataset'):
                text = tokenizer(dataset[sst5_idx]['sentence'], padding='max_length', max_length=max_length, truncation=True)
                dataset_dict['item_idx'].append(idx)
            ...
            dataset = pd.read_json(os.path.join(args.data_dir,"hatexplain","processed_"+split+".json"), orient='records', lines=True)
    ...
    if args.pct_train_rationales is not None:
        ...
        if args.train_rationale_selection == 'uniform':
            select_rationale_indices(data_path, dataset_dict, num_examples, 'uniform', args.pct_train_rationales, args.seed)
        else:
            raise NotImplementedError
        sys.exit()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pct_train_rationales', type=float, help='Percentage of train examples to provide gold rationales for. None means all available train examples are used.')
    parser.add_argument('--train_rationale_selection', type=str, help='Strategy for selecting which train examples to annotate rationales for.', choices=['uniform'])
    parser.add_argument('--seed', type=int, default=0, help='Random seed')
    parser.add_argument('--anno_method',type=str,default='normal',choices=['normal','lexicon_based_all','lexicon_based','instance_based'])
